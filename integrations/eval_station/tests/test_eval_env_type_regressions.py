...
class EvalEnvTypeRegressionTests(unittest.TestCase):
    def test_env_client_server_exports_error_normalizer(self):
        from eval_station.servers import env_client_server

        self.assertTrue(callable(env_client_server.normalize_execution_error))

    def test_default_runner_uses_sim_runner_for_sim(self):
        runner = importlib.import_module("eval_station.env_client.runner")
    ...
    def test_reset_deploy_cfg_includes_repeat_index(self):
        ...
        from eval_station.env_client.api import EnvClientBaselineConfig
        from eval_station.env_client.runner import baseline_to_reset_deploy_cfg
    ...
    def test_failed_runner_construction_releases_trial_registration(self):
        ...
        from eval_station.servers import env_client_server as ecs

        state = ecs.EnvClientServerState(
            baseline=EnvClientBaselineConfig(eval_env_type="sim"),
            config=ecs.EnvClientServerConfig(
        ...
        original = ecs.make_dispatch_trial_runner
        ecs.make_dispatch_trial_runner = _boom
        try:
            with self.assertRaises(TrialRunnerError):
                state.trial_runner_with_stop("eval-1", 1)
        finally:
            ecs.make_dispatch_trial_runner = original
