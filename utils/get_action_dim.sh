...
env_cfg = yaml.safe_load(
    open(os.path.join(root_dir, "XPolicyLab", "env_cfg", f"{env_cfg_type}.yml"), "r", encoding="utf-8")
)
robot_name = env_cfg["config"]["robot"]
robot_action_dim_info = json.load(
    open(os.path.join(root_dir, "XPolicyLab", "env_cfg", "robot", "_robot_info.json"), "r", encoding="utf-8")
)[robot_name]
