...
#   HDF5_DATA_ROOT         Raw HDF5 tree used by relative episode paths in meta
#   MAX_STEPS             Optimizer steps; defaults to 50000
...
: "${HDF5_DATA_ROOT:?Set HDF5_DATA_ROOT to the raw Fold Shirt HDF5 data tree}"
export DATA_ROOT
export HDF5_DATA_ROOT
export WSP_TASK="fold-shirt-text-stage1"
...
if [[ ! -d "$HDF5_DATA_ROOT" ]]; then
    echo "ERROR: Raw HDF5 data root not found at $HDF5_DATA_ROOT"
    exit 1
fi

exec "$ROOT/recipes/common/posttrain_agilex.sh" \
    "data_loader.dataset_kwargs.data_roots=[\"${HDF5_DATA_ROOT}\"]" \
    "$@"
