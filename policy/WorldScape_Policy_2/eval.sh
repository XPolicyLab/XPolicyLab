...
warp_cache_root="${ROBOTWIN_WARP_CACHE_ROOT:-${TMPDIR:-/tmp}/robotwin-warp-cache-${USER:-user}}"
export WARP_CACHE_PATH="${warp_cache_root}/gpu-${env_gpu_id}"
echo "[${policy_name}] WARP_CACHE_PATH=${WARP_CACHE_PATH}"
...
cleanup() {
    if [[ -n "${SERVER_PID:-}" ]]; then
        echo "[CLEANUP] kill server process group ${SERVER_PID}"
