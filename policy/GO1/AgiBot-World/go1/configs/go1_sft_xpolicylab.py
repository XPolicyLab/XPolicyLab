"""GO-1 fine-tuning config for XPolicyLab datasets (dual-arm, joint action type)."""
...
RUNNAME = os.environ.get("RUNNAME", "go1_xpolicylab")
DEBUG_MODE = get_bool_env("DEBUG_MODE")

# These are injected via environment variables from train.sh
DATA_ROOT_DIR = os.environ.get("DATA_ROOT_DIR", "/path/to/your/lerobot/dataset")
ACTION_DIM = int(os.environ.get("ACTION_DIM", "14"))
STATE_DIM = int(os.environ.get("STATE_DIM", "14"))
CTRL_FREQ = int(os.environ.get("CTRL_FREQ", "30"))
...
LOGGING_DIR = os.environ.get("LOGGING_DIR", os.path.join(os.path.dirname(__file__), "../../..", "checkpoints", RUNNAME, "tb"))
...
class SpaceArguments(BaseSpaceArguments):
    ...
    space_repack: dict = field(
        default_factory=lambda: {
            ...
            "action": "action",
            "cam_head_color": "observation.images.cam_head",
            "cam_hand_left_color": "observation.images.cam_hand_left",
            "cam_hand_right_color": "observation.images.cam_hand_right",
        }
    )
