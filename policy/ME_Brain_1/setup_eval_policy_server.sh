...
if [[ ! -f "${policy_uv_env_path}/.venv/bin/activate" ]]; then
    ...
    echo "[SERVER][ERROR] Create the dedicated environment documented in the repository README." >&2
    exit 1
fi
...
POLICY_ENV_SRC="${policy_uv_env_path}/src"
echo "[SERVER] Using python: ${PYTHON_BIN}"

PYTHONPATH_PARTS=("${BENCH_ROOT}")
if [[ -d "${POLICY_ENV_SRC}" ]]; then
    PYTHONPATH_PARTS+=("${POLICY_ENV_SRC}")
fi
