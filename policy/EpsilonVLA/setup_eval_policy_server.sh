...
# 官方 get_action_dim 只读 yaml/json；用 conda python，避免系统 python 缺依赖。
action_dim="$(
  GET_ACTION_DIM_PYTHON="${YAML_PYTHON}" \
    bash "${UTILS_DIR}/get_action_dim.sh" "${BENCH_ROOT}" "${env_cfg_type}"
)"
...
EPSILONVLA_SRC="${policy_uv_env_path}/src"
echo "[SERVER] Using python: ${PYTHON_BIN}"
echo "[SERVER] EPSILONVLA_ROOT=${policy_uv_env_path}"
echo "[SERVER] XLA_PYTHON_CLIENT_MEM_FRACTION=${XLA_PYTHON_CLIENT_MEM_FRACTION:-0.3}"

PYTHONPATH_PARTS=("${XPL_ROOT}")
if [[ -d "${EPSILONVLA_SRC}" ]]; then
    PYTHONPATH_PARTS+=("${EPSILONVLA_SRC}")
fi
