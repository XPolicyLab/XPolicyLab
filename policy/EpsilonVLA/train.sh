...
train_config_name="${EPSILONVLA_TRAIN_CONFIG_NAME:-epsilonvla_robodojo_posttrain_abs_joint_infer}"
lerobot_repo_id="${EPSILONVLA_LEROBOT_REPO_ID:-${bench_name}-${ckpt_name}-${env_cfg_type}-${action_type}}"
gpu_count=$(awk -F',' '{print NF}' <<<"${gpu_id}")
fsdp_devices="${EPSILONVLA_FSDP_DEVICES:-$(( gpu_count < 2 ? 1 : 2 ))}"
...
LOCAL_CACHE_ROOT="${EPSILONVLA_LOCAL_CACHE_ROOT:-/tmp/epsilonvla-cache-$(hostname)}"
...
echo "[EpsilonVLA] train_config_name=${train_config_name}"
echo "[EpsilonVLA] lerobot_repo_id=${lerobot_repo_id}"
echo "[EpsilonVLA] fsdp_devices=${fsdp_devices}"
echo "[EpsilonVLA] local_cache_root=${LOCAL_CACHE_ROOT}"
echo "[EpsilonVLA] checkpoint_dir=${ckpt_dir}"

cd "${POLICY_DIR}/epsilonvla/"
