...
# Running epsilonvla models remotely

We provide utilities for running epsilonvla models remotely. This is useful for running inference on more powerful GPUs off-robot, and also helps keep the robot and policy environments separate (and e.g. avoid dependency hell with robot software).
...
```bash
uv run scripts/serve_policy.py policy:checkpoint --policy.config=pi0_fast_droid --policy.dir=gs://epsilonvla-assets/checkpoints/pi0_fast_droid
```
...
First, install the `epsilonvla-client` package in your robot environment:

```bash
cd $EPSILONVLA_ROOT/packages/epsilonvla-client
pip install -e .
```
...
```python
from epsilonvla_client import image_tools
from epsilonvla_client import websocket_client_policy
