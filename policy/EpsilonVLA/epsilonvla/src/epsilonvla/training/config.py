...
import epsilonvla.models.pi0_config as pi0_config
import epsilonvla.models.pi0_fast as pi0_fast
import epsilonvla.models.tokenizer as _tokenizer
import epsilonvla.policies.agilex_policy as agilex_policy
import epsilonvla.policies.aloha_policy as aloha_policy
import epsilonvla.policies.droid_policy as droid_policy
import epsilonvla.policies.libero_policy as libero_policy
import epsilonvla.policies.wuji_policy as wuji_policy
import epsilonvla.shared.download as _download
import epsilonvla.shared.normalize as _normalize
import epsilonvla.training.droid_rlds_dataset as droid_rlds_dataset
import epsilonvla.training.misc.polaris_config as polaris_config
import epsilonvla.training.misc.roboarena_config as roboarena_config
import epsilonvla.training.optimizer as _optimizer
import epsilonvla.training.weight_loaders as weight_loaders
import epsilonvla.transforms as _transforms

# RoboDojo normalization assets bundled with this adapter (epsilonvla/assets/RoboDojo_assets).
...
class AssetsConfig:
    ...
    ```
    AssetsConfig(
        assets_dir="gs://epsilonvla-assets/checkpoints/pi0_base/assets",
        asset_id="trossen",
    )
    ```
    """
...
class DataConfig:
    ...
    # Optional local root directory for the dataset. This is only used by data
    # loaders, but keeping it here lets eval reuse training configs verbatim.
    root: str | None = None
...
class DataConfigFactory(abc.ABC):
    ...
    # Optional local root directory for the dataset.
    root: str | None = None
...
class LerobotAgilexDataConfig(DataConfigFactory):
    ...
    Kept in sync with the kai0 training repo so that a checkpoint trained there is served
    through an identical transform pipeline here.
    """
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # if prompt_from_task is True, add prompt to the repacked structure
...
class LeRobotWujiDataConfig(DataConfigFactory):
    """
    Config for Tianji Marvin + dual Wuji Hand Gen1 (adapted from wuji-epsilonvla).
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # Arms: delta; hands: absolute (same as wuji-epsilonvla).
        if self.extra_delta_transform:
...
class RLDSDroidDataConfig(DataConfigFactory):
    ...
    datasets: Sequence[droid_rlds_dataset.RLDSDataset] = (
        droid_rlds_dataset.RLDSDataset(
            ...
            version="1.0.1",
            weight=1.0,
            filter_dict_path="gs://epsilonvla-assets/droid/droid_sample_ranges_v1_0_1.json",
        ),
    )
...
_CONFIGS = [
    TrainConfig(
        ...
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://epsilonvla-assets/checkpoints/pi05_base/params"),
        seed=0,
        batch_size=256,
        fsdp_devices=2,
        num_train_steps=60000,
    ),
    # Absolute-action fine-tuning from the official RoboDojo Pi05 checkpoint.
    # The dataset-provided stats are absolute-action stats, so delta conversion
    # must remain disabled for both training and inference.

    # Shared eval config for RoboDojo post-train checkpoints (tasks 16/23/30, official + self-collected).
    # Prompt comes from the environment ``instruction`` field; only the checkpoint path differs.
    TrainConfig(
        name="pi05_robodojo_posttrain_infer",
        model=pi0_config.Pi0Config(pi05=True),
        data=LerobotAgilexDataConfig(
            repo_id="RoboDojo_lerobot_v21_video",
            root="/pfs/public/RoboDojo_lerobot_v21_video",
            assets=AssetsConfig(assets_dir="/pfs/public", asset_id="RoboDojo_lerobot_v21_video"),
    ...
    TrainConfig(
            name="epsilonvla_robodojo_posttrain_abs_joint_infer",
            model=pi0_config.Pi0Config(pi05=True),
            data=LerobotAgilexDataConfig(
                repo_id="RoboDojo_lerobot_v21_video",
                root="/pfs/public/RoboDojo_lerobot_v21_video",
    ...
    TrainConfig(
        ...
        weight_loader=weight_loaders.PartialCheckpointWeightLoader(
            "gs://epsilonvla-assets/checkpoints/pi05_base/params"
        ),
