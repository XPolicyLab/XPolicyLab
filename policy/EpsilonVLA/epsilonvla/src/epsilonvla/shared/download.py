...
# Environment variable to control cache directory path, ~/.cache/epsilonvla will be used by default.
_EPSILONVLA_DATA_HOME = "EPSILONVLA_DATA_HOME"
DEFAULT_CACHE_DIR = "~/.cache/epsilonvla"

logger = logging.getLogger(__name__)


def get_cache_dir() -> pathlib.Path:
    cache_dir = pathlib.Path(os.getenv(_EPSILONVLA_DATA_HOME, DEFAULT_CACHE_DIR)).expanduser().resolve()
    ...
    try:
        ...
        with filelock.FileLock(lock_path):
            ...
            if not local_path.exists():
                ...
                # Route epsilonvla-assets through gsutil to avoid gcsfs auth issues with this bucket.
                # All other gs:// URLs (e.g. big_vision) continue to use gcsfs as normal.
                if parsed.scheme == "gs" and parsed.netloc == "epsilonvla-assets":
...
_INVALIDATE_CACHE_DIRS: dict[re.Pattern, float] = {
    re.compile("epsilonvla-assets/checkpoints/pi0_aloha_pen_uncap"): _get_mtime(2025, 2, 17),
    re.compile("epsilonvla-assets/checkpoints/pi0_libero"): _get_mtime(2025, 2, 6),
    re.compile("epsilonvla-assets/checkpoints/"): _get_mtime(2025, 2, 3),
}
