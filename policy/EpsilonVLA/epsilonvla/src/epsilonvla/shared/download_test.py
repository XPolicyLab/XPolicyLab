...
import epsilonvla.shared.download as download


@pytest.fixture(scope="session", autouse=True)
def set_epsilonvla_data_home(tmp_path_factory):
    temp_dir = tmp_path_factory.mktemp("epsilonvla_data")
    with pytest.MonkeyPatch().context() as mp:
        mp.setenv("EPSILONVLA_DATA_HOME", str(temp_dir))
        yield
...
def test_download_gs_dir():
    remote_path = "gs://epsilonvla-assets/testdata/random"
...
def test_download_gs():
    remote_path = "gs://epsilonvla-assets/testdata/random/random_512kb.bin"
