...
from epsilonvla.models import tokenizer as _tokenizer
...
class Normalize(DataTransformFn):
    ...
    def _normalize(self, x, stats: NormStats):
        dim = x.shape[-1]
        mean = pad_to_dim(stats.mean, dim, axis=-1, value=0.0)[..., :dim]
        std = pad_to_dim(stats.std, dim, axis=-1, value=1.0)[..., :dim]
        return (x - mean) / (std + 1e-6)

    def _normalize_quantile(self, x, stats: NormStats):
        assert stats.q01 is not None
        assert stats.q99 is not None
        # Stats may cover fewer dims than the zero-padded model input. Those dims normalize to -1,
        # which is what stats computed on already-padded data produce.
        dim = x.shape[-1]
        q01 = pad_to_dim(stats.q01, dim, axis=-1, value=0.0)[..., :dim]
        q99 = pad_to_dim(stats.q99, dim, axis=-1, value=1.0)[..., :dim]
