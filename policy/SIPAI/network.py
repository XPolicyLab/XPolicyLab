...
class Prefix(nn.Module):
    def __init__(self, history_grid_size, history_compressor_hidden_dim):
        super().__init__()
        config = GemmaConfig(2048, 18, 16384, 8, 1, 256)
        self.vision_encoder = SigLIPVisionEncoder()
        self.history_compressor = HistoryEncoder(history_grid_size, history_compressor_hidden_dim)
        ...
        self.history_grid_size = history_grid_size
    ...
    def forward(self, images, history, history_mask, token_ids, token_mask):
        ...
        history_tokens = grid_size * grid_size
        if indices.numel():
            encoded = self.vision_encoder(history.flatten(0, 1).index_select(0, indices))
            encoded = self.history_compressor(encoded)
            scattered = encoded.new_zeros(batch * frames, history_tokens, 2048)
            scattered = scattered.index_copy(0, indices, encoded)
        else:
            scattered = parts[0].new_zeros(batch * frames, history_tokens, 2048)
        parts.insert(0, scattered.reshape(batch, frames * history_tokens, 2048))
        masks.insert(0, history_mask[:, :, None].expand(-1, -1, history_tokens).reshape(batch, -1))
