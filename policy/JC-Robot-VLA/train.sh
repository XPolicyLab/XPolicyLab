...
if [[ $# -lt 6 ]]; then
  echo "Usage: $0 <bench_name> <ckpt_name> <env_cfg_type> <action_type> <seed> <gpu_id> [extra train.py args...]" >&2
  exit 1
fi
...
echo "[JC-Robot-VLA] train_config_name=${train_config_name}"
echo "[JC-Robot-VLA] lerobot_repo_id=${lerobot_repo_id}"
echo "[JC-Robot-VLA] fsdp_devices=${fsdp_devices}"
echo "[JC-Robot-VLA] local_cache_root=${LOCAL_CACHE_ROOT}"
echo "[JC-Robot-VLA] checkpoint_dir=${ckpt_dir}"
if [[ ${#extra_args[@]} -gt 0 ]]; then
  echo "[JC-Robot-VLA] extra_args=${extra_args[*]}"
fi
