"""
XPolicyLab Deploy Module — JC-Robot-VLA.

Adds inference-time optimizations (no retraining required):
  - action_chunk_exec_steps: limit open-loop execution per chunk (closed-loop control)
"""
...
def _read_deploy_cfg(TASK_ENV) -> dict:
    """Read deploy.yml for this policy to access optimization parameters."""
    policy_name = getattr(TASK_ENV, "deploy_cfg", {}).get("policy_name", None)
    if policy_name is None:
        return {}
    deploy_yml = _POLICY_DIR / "deploy.yml"
...
def eval_one_episode(TASK_ENV, model_client):
    ...
    cfg = _read_deploy_cfg(TASK_ENV)
    exec_steps = cfg.get("action_chunk_exec_steps", None)
    ...
    while not TASK_ENV.is_episode_end(): # Check whether the episode ends
        ...
        # Determine how many actions to execute before re-inferring
        exec_limit = len(actions)
        if exec_steps is not None:
            exec_limit = min(int(exec_steps), len(actions))

        for action_idx, action in enumerate(actions):
            TASK_ENV.take_action(action)

            if TASK_ENV.is_episode_end() or action_idx + 1 >= exec_limit:
                break
...
def eval_one_episode_batch(TASK_ENV, model_client):
    ...
    while not TASK_ENV.is_episode_end(): # Check whether the episode ends
        ...
        exec_limit = chunk_size
        if exec_steps is not None:
            exec_limit = min(int(exec_steps), chunk_size)
        ...
        for action_idx in range(chunk_size): # Iterate over the action chunk
            ...
            if TASK_ENV.is_episode_end() or action_idx + 1 >= exec_limit: # Check whether the episode ends
                break
