...
SEED="${SEED:-930}"
RUN_ID="${RUN_ID:-all_tasks_videopt_8g_1epoch_$(date +%m%d_%H%M%S)}"

DATASET_NAME="${DATASET_NAME:-RoboDojo_lerobot_v21_video}"
CKPT_NAME="${CKPT_NAME:-all_tasks_videopt}"
...
export GIGAWORLD_CONFIG="${GIGAWORLD_CONFIG:-configs.robodojo_all_tasks_videopt_epoch1.config}"
...
{
  echo "[GigaWorldPolicy] RoboDojo all-task video pretrain"
  echo "  run_id:     ${RUN_ID}"
