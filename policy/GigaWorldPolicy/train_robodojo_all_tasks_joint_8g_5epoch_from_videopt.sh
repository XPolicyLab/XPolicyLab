...
SEED="${SEED:-930}"
RUN_ID="${RUN_ID:-all_tasks_joint5ep_from_videopt_$(date +%m%d_%H%M%S)}"

DATASET_NAME="${DATASET_NAME:-RoboDojo_lerobot_v21_video}"
CKPT_NAME="${CKPT_NAME:-all_tasks_joint5ep_from_videopt}"
...
export GIGAWORLD_CONFIG="${GIGAWORLD_CONFIG:-configs.robodojo_all_tasks_joint_epoch5_from_videopt.config}"
...
export GIGAWORLD_WANDB_PROJECT="${GIGAWORLD_WANDB_PROJECT:-gwp-robodojo}"
...
{
  echo "[GigaWorldPolicy] RoboDojo all-task joint fine-tune from all-task video-pt"
  echo "  run_id:       ${RUN_ID}"
