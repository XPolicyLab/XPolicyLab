...
def train_step(
    ...
    future_targets: at.Float[at.Array, "b t d"],
    future_mask: at.Bool[at.Array, "b t"],
    ...
    def loss_fn(
        ...
        loss, action_loss, align_loss, future_loss = model.compute_align_losses(
            rng,
            observation,
            actions,
            align_targets,
            align_mask,
            future_targets if config.future_enabled else None,
            future_mask if config.future_enabled else None,
            train=True,
        )
        return loss, (action_loss, align_loss, future_loss)

    train_rng = jax.random.fold_in(rng, state.step)
    observation, actions = batch

    diff_state = nnx.DiffState(0, config.trainable_filter)
    (loss, (action_loss, align_loss, future_loss)), grads = nnx.value_and_grad(
...
def _unpack_align_batch(batch):
    # RD_WAM: 4-tuple carries (observation, actions, sf_identity, future).
    if len(batch) == 4:
        return batch
    if len(batch) == 3:
        observation, actions, sf_identity = batch
        return observation, actions, sf_identity, None
    observation, actions = batch
...
def main(config: _config.TrainConfig):
    ...
    # RD_JAX_CACHE: prefer JAX_COMPILATION_CACHE_DIR (set to node-local

    # scratch by sf_paths.sh); the original hardcoded ~/.cache/jax, which

    # env.sh points at shared Lustre.
    ...
    observation, actions, sf_identity, future = _unpack_align_batch(next(data_iter))
    batch = (observation, actions)
    logging.info(f"Initialized data loader:\n{training_utils.array_tree_to_info(batch)}")

    # RD_WAM
    future_enabled = bool(getattr(config, "future_enabled", False))
    if future_enabled and future is None:
        raise ValueError("future_enabled=True but the data loader returned no future frames.")
    future_tokens = 0
    if future_enabled:
        fshape = future["image"].shape
        future_tokens = (fshape[1] // 14) * (fshape[2] // 14)
        logging.info(f"RD_WAM future prediction on, {future_tokens} target tokens per sample")
    ...
    def _submit_batch(obs, ident, fut):
        align_worker.submit(
            align_utils.prepare_align_observation(obs),
            reference_tokens=reference_tokens,
            sf_identity=None if ident is None else jax.device_get(ident),
        )
        if future_enabled:
            align_worker.submit(
                align_utils.prepare_future_observation(fut),
                reference_tokens=future_tokens,
            )

    # RD_WAM: constant placeholders keep ptrain_step's arity fixed when the
    # future head is off (the model never reads them in that case).
    dummy_future = align_utils.AlignFeatures(
        targets=np.zeros((config.batch_size, 1, 1), dtype=np.float32),
        mask=np.zeros((config.batch_size, 1), dtype=np.bool_),
    )

    _submit_batch(observation, sf_identity, future)
    align_features = align_worker.result()
    future_features = align_worker.result() if future_enabled else dummy_future
    ...
    try:
        for step in pbar:
            next_observation, next_actions, next_sf_identity, next_future = _unpack_align_batch(next(data_iter))
            _submit_batch(next_observation, next_sf_identity, next_future)

            feature_wait_start = time.perf_counter()
            align_targets, align_mask = _shardalign_utils(align_features, data_sharding)
            future_targets, future_mask = _shardalign_utils(future_features, data_sharding)
            feature_wait_s = time.perf_counter() - feature_wait_start

            train_start = time.perf_counter()
            with sharding.set_mesh(mesh):
                train_state, info = ptrain_step(
                    train_rng, train_state, batch, align_targets, align_mask, future_targets, future_mask
                )
