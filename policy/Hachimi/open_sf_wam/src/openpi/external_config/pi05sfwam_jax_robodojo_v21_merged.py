# RD_WAM: Spatial Forcing + action-conditioned future latent prediction head,
# trained on the MERGED dataset (3500 released episodes + self-generated
# general_pickup episodes with per-episode instructions). Additive copy of
# pi05sf_jax_robodojo_v21_merged with the future head enabled; everything else
# (data, optimizer, schedule, batch, alignment) is identical, so this run and
# sf_merged_v1 differ by exactly one mechanism.
#
# The head only exists at training time: sample_actions never calls it, and the
# deploy path is unchanged apart from train_config_name/repo_id overrides.
import os
...
my_config = TrainConfig(
    name="pi05sfwam_jax_robodojo_v21_merged",
    project_name="xpolicylab-pi05sf-jax",
    model=pi0_config.Pi0Config(pi05=True),
    data=LeRobotAlohaDataConfig(
        repo_id="RoboDojo_lerobot_v21_merged",
        assets=AssetsConfig(
            assets_dir=PI05SF_ASSETS_DIR,
            asset_id="RoboDojo_lerobot_v21_merged",
        ),
        adapt_to_pi=False,
    ...
    sf_dataset_uid=0,
    future_enabled=True,
    future_loss_coeff=0.05,
    future_offset_s=2.0,
    future_source_key="observation.images.cam_high",
    seed=0,
    batch_size=256,
    num_workers=8,
