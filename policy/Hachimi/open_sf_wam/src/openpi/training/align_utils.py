...
class _TorchAlignFeatureShard:
    ...
    def _extract_features(self, observation: _model.Observation, reference_tokens: int) -> AlignFeatures:
        with torch.cuda.device(self.device) if self.device.type == "cuda" else contextlib.nullcontext():
            ...
            # RD_SINGLE_LAYER_POOL: pool only the layer that is kept. Every
            # call takes the same arguments apart from `hidden`, so indexing the
            # input is identical to indexing the pooled output, minus 23/24 of
            # the compute and ~9.6 GB of retained float32 activations.
            target = custom_pooling(
                vggt_hidden[self.config.vggt_layers_align],
                (patch_h, patch_w),
                (h, w),
                reference,
...
class AsyncAlignFeatureWorker:
    def __init__(self, extractor):
        ...
        # RD_WAM: FIFO of outstanding jobs so one step can queue several feature
        # extractions (current obs + future frame). max_workers=1 keeps them
        # serialized on the VGGT devices; result() pops in submit order.
        self.futures: collections.deque[concurrent.futures.Future[AlignFeatures]] = collections.deque()
    ...
    def result(self) -> AlignFeatures:
        ...
        return self.futures.popleft().result()
...
def prepare_future_observation(future: dict[str, Any]) -> _model.Observation:
    """RD_WAM: wrap the future base-camera frame as a single-image Observation.

    The extractor only touches images / image_masks / image_padding_mask, so a
    dummy state is enough. image_masks encodes lerobot's episode-boundary
    padding (a clamped future frame contributes no loss), image_padding_mask the
    letterbox pixels from resize_with_pad.
    """
    image = np.asarray(jax.device_get(future["image"]), dtype=np.float32)
    pixel_mask = np.asarray(jax.device_get(future["pixel_mask"]), dtype=bool)
    is_pad = np.asarray(jax.device_get(future["is_pad"]), dtype=bool).reshape(-1)
    return _model.Observation(
        images={"future_base": image},
        image_masks={"future_base": ~is_pad},
        image_padding_mask={"future_base": pixel_mask},
        state=np.zeros((image.shape[0], 1), dtype=np.float32),
    )
