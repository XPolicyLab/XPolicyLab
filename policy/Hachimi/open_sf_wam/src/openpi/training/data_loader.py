...
import numpy as np
import torch
from openpi_client import image_tools as _client_image_tools
...
# RD_WAM: future-frame fields produced by FutureFrameSplitter. They are carried
# around the transform chain (RepackTransform would drop them) the same way the
# SF identity fields already are.
_FUTURE_FIELDS = ("future_frame", "future_frame_pixel_mask", "future_frame_is_pad")
_PASSTHROUGH_FIELDS = _SF_IDENTITY_FIELDS + _FUTURE_FIELDS
...
class TransformedDataset(Dataset[T_co]):
    ...
    def __getitem__(self, index: SupportsIndex) -> T_co:
        ...
        identity = {field: sample[field] for field in _PASSTHROUGH_FIELDS if field in sample}
...
class FutureFrameSplitter(Dataset):
    """RD_WAM: split the stacked (t, t + future_offset) source video frames.

    The source key was loaded with delta_timestamps [0, future_offset_s], so it
    arrives as [2, C, H, W]. Frame 0 is put back so the standard chain sees the
    exact sample it always saw; frame 1 is processed here to its final batch
    form (mirroring _decode_aloha + ResizeImages + Observation.from_dict):
    uint8 CHW -> HWC -> resize_with_pad(224) with pixel padding mask -> float32
    in [-1, 1]. lerobot's `<key>_is_pad` marks futures clamped at the episode
    boundary; it becomes future_frame_is_pad so the loss can mask them.
    """

    def __init__(self, dataset: Dataset, source_key: str, height: int = 224, width: int = 224):
        self._dataset = dataset
    ...
    def __getitem__(self, index: SupportsIndex) -> dict:
        ...
        frames = sample[self._source_key]
        frames_np = np.asarray(frames)
        if frames_np.ndim != 4 or frames_np.shape[0] != 2:
            raise ValueError(
                f"FutureFrameSplitter expects {self._source_key} as [2, C, H, W], got {frames_np.shape}."
            )
        sample[self._source_key] = frames[0]

        future = frames_np[1]
        if np.issubdtype(future.dtype, np.floating):
            future = (255 * future).astype(np.uint8)
        future = np.transpose(future, (1, 2, 0))
        resized, pixel_mask = _client_image_tools.resize_with_pad(
            future, self._height, self._width, return_mask=True
        )
        sample["future_frame"] = np.asarray(resized, dtype=np.float32) / 255.0 * 2.0 - 1.0
        sample["future_frame_pixel_mask"] = np.asarray(pixel_mask, dtype=bool)

        is_pad = sample.pop(f"{self._source_key}_is_pad", None)
        sample["future_frame_is_pad"] = np.asarray(
            False if is_pad is None else np.asarray(is_pad).reshape(-1)[-1], dtype=bool
        )
        return sample
...
def create_torch_dataset(
    ...
    *,
    future_offset_s: float | None = None,
    future_source_key: str | None = None,
) -> Dataset:
    ...
    # RD_WAM: also load the base-camera frame at t + future_offset_s. lerobot
    # clamps past the episode end and reports it via <key>_is_pad.
    if future_offset_s is not None and future_source_key is not None:
        delta_timestamps[future_source_key] = [0.0, float(future_offset_s)]
    dataset = lerobot_dataset.LeRobotDataset(
        data_config.repo_id,
        delta_timestamps=delta_timestamps,
    )

    # RD_DROP_UNUSED_VIDEO_KEYS: lerobot decodes every feature whose dtype is
    # "video" (LeRobotDataset._query_videos over meta.video_keys). The RoboDojo
    # v2.1 export carries four camera streams while the Spatial-Forcing repack
    # reads only three, so the fourth is decoded and thrown away on every sample
    # (measured 0.066 s/sample with four vs 0.042 s with three). Drop unused
    # streams from the metadata before any __getitem__ runs.
    _wanted = set()
    for _t in data_config.repack_transforms.inputs:
        _struct = getattr(_t, "structure", None)
        if isinstance(_struct, dict):
            for _v in _struct.get("images", {}).values():
                _wanted.add(_v)
    if _wanted:
        for _key in [k for k in dataset.meta.video_keys if k not in _wanted]:
            dataset.meta.info["features"].pop(_key, None)
            logging.info("dropping unused video stream %s", _key)

    if future_offset_s is not None and future_source_key is not None:
        dataset = FutureFrameSplitter(dataset, future_source_key)
...
def create_transformed_single_torch_dataset(
    ...
    *,
    ...
    dataset = create_torch_dataset(
        ...
        future_source_key=future_source_key,
    )
...
def create_data_loader(
    ...
    *,
    ...
    return create_torch_data_loader(
        ...
        return_sf_identity=config.sf_cache_enable,
        sf_dataset_uid=config.sf_dataset_uid,
        future_offset_s=config.future_offset_s if getattr(config, "future_enabled", False) else None,
        future_source_key=config.future_source_key if getattr(config, "future_enabled", False) else None,
    )
...
def create_torch_data_loader(
    ...
    *,
    ...
    """
    return_future = future_offset_s is not None and future_source_key is not None
    if isinstance(data_config, _config.MixedDataConfig):
        if return_future:
            raise NotImplementedError("RD_WAM future frames are not supported for MixedDataConfig.")
    ...
    def __iter__(self):
        for batch in self._data_loader:
            ...
            if self._return_future:
                # RD_WAM: 4-tuple contract (observation, actions, sf_identity, future).
                identity = extract_sf_identity(batch, self._dataset_uid) if self._return_sf_identity else None
                future = {
                    "image": batch["future_frame"],
                    "pixel_mask": batch["future_frame_pixel_mask"],
                    "is_pad": batch["future_frame_is_pad"],
                }
                yield observation, actions, identity, future
            elif self._return_sf_identity:
