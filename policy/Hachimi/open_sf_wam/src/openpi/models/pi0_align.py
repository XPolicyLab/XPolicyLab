...
class Pi0Align(pi0.Pi0):
    def __init__(self, config: Any, rngs: nnx.Rngs):
        ...
        # RD_WAM: optional action-conditioned future latent prediction head.
        self.future_enabled = bool(getattr(config, "future_enabled", False))
        if self.future_enabled:
            self.future_predictor = projectors.FuturePredictor(
            ...
            self.future_loss_coeff = config.future_loss_coeff
    ...
    def compute_align_losses(
        ...
        future_targets: jax.Array | None = None,
        future_mask: jax.Array | None = None,
        *,
        train: bool = False,
        ...
        if self.future_enabled and future_targets is not None and future_mask is not None:
            # The base camera is embedded first (AlohaInputs puts base_0_rgb
            # first and dict order is preserved), so its layer-N hiddens are the
            # leading tokens_per_img slice of the captured vision hiddens.
            tokens_per_img = image_token_count // len(observation.images)
            base_tokens = vision_hidden[:, :tokens_per_img]
            future_loss = self.future_predictor(base_tokens, actions, future_targets, future_mask)
            loss = loss + self.future_loss_coeff * future_loss
        else:
            future_loss = jnp.zeros((), dtype=jnp.float32)
