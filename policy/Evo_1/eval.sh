...
if [[ -n "${EVO1_EVAL_TASK_CONFIG:-}" ]]; then
    additional_info+=",task_config=${EVO1_EVAL_TASK_CONFIG}"
fi
if [[ -n "${EVO1_EVAL_TEST_NUM:-}" ]]; then
    additional_info+=",test_num=${EVO1_EVAL_TEST_NUM}"
fi
