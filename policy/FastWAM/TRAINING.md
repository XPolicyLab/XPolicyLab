...
bash XPolicyLab/policy/FastWAM/process_data.sh \
    RoboDojo test_data arx_x5 joint
# ablation：只取 3 条，换一个 ckpt_name 区分
bash XPolicyLab/policy/FastWAM/process_data.sh \
    RoboDojo test_data_n3 arx_x5 joint 3 test_data
```

参数顺序与 XPolicyLab 全家桶保持一致：
`bench_name ckpt_name env_cfg_type action_type [expert_data_num] [raw_task_dirs] [dataset_id]`。
`expert_data_num` 可选（留空 = 全部 episode）；`raw_task_dirs` 缺省为 `${ckpt_name}`。
默认 `dataset_id = <bench_name>-<ckpt_name>-<env_cfg_type>-<action_type>`。

### 4.2 多任务 cotrain（一次合并多个 task 到同一个 LeRobot 数据集）

```bash
# 方式 A：raw_task_dirs 传任务列表（逗号分隔），expert_data_num 是“每个 task 各取多少 episode”
bash XPolicyLab/policy/FastWAM/process_data.sh \
    RoboDojo cotrain arx_x5 joint 3 "stack_bowls,press_by_number"

# 方式 B：自动发现 final_data/<dataset>/*/<env>/data/episode_*.hdf5 下的所有 task
bash XPolicyLab/policy/FastWAM/process_data_batch.sh \
    RoboDojo cotrain arx_x5 joint 3
```

可选尾参 `dataset_id` 控制输出文件夹名（默认 4 元组 `<bench>-<ckpt>-<env>-<action>`）。
...
```bash
# 多卡 + cotrain（前提是已 process_data_batch 以 ckpt_name=cotrain 合并多个 task）
bash XPolicyLab/policy/FastWAM/train.sh \
...
- 训练时 `<ckpt_name>` 参与拼 dataset 文件夹名与 checkpoint 目录名；找原始 episode 目录用
  `raw_task_dirs`（缺省等于 `ckpt_name`）。
