...
from galaxea_fm.data.lerobot.datasets.compute_stats import aggregate_stats, compute_episode_stats
from galaxea_fm.data.lerobot.datasets.util_v3 import (
...
from galaxea_fm.data.lerobot.datasets.video_utils import (
    VideoFrame,
...
from galaxea_fm.data.lerobot.constants import HF_LEROBOT_HOME

CODEBASE_VERSION = "v3.0"


def write_image(image, fpath: Path) -> None:
    """Write an image (numpy HWC uint8 or PIL.Image) to disk.

    NOTE: upstream commented out the lerobot image_writer import but still calls
    write_image in LeRobotDataset._save_image, so it is defined here to avoid a
    NameError during add_frame (synchronous, image_writer disabled).
    """
    try:
        if isinstance(image, np.ndarray):
            ...
            raise TypeError(f"Unsupported image type for write_image: {type(image)}")
        img.save(str(fpath))
    except Exception as e:  # noqa: BLE001
        logging.error(f"Failed to write image to {fpath}: {e}")
        raise
...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def save_episode(self, episode_data: dict | None = None) -> None:
        ...
        # Galaxea-specific default index features (coarse_task/coarse_quality/quality)
        # are part of DEFAULT_FEATURES but are NOT populated by add_frame; default them
        # to zeros so the np.stack below does not fail on an empty buffer.
        _unpopulated_default_keys = ("coarse_task_index", "coarse_quality_index", "quality_index")
        for _key in _unpopulated_default_keys:
            if _key in self.features and len(episode_buffer.get(_key, [])) == 0:
                episode_buffer[_key] = np.zeros((episode_length,), dtype=np.int64)

        for key, ft in self.features.items():
            # index, episode_index, task_index are already processed above, and image and video
            # are processed separately by storing image path and frame info as meta data
            if (
                key in ["index", "episode_index", "task_index", *_unpopulated_default_keys]
    ...
    def _encode_temporary_episode_video(self, video_key: str, episode_index: int) -> Path:
        ...
        # Use h264 (not the libsvtav1 default): torchcodec in this environment
        # cannot decode AV1 ("Function not implemented"), while h264 is decodable
        # by both torchcodec and pyav.
        encode_video_frames(img_dir, temp_path, self.fps, vcodec="h264", overwrite=True)
