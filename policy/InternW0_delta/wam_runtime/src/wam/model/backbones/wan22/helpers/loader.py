...
WAN22_MODEL_REGISTRY = [
    ...
    {
        # robbyant/lingbot-va-base transformer: Diffusers naming with extra
        # action-conditioning weights. The converter selects the complete
        # Video DiT backbone and ignores those action-only parameters.
        "model_hash": "2170884431d54fc35bde88f2a9efe1e4",
        "model_name": "wan_video_dit",
        "model_class": WanVideoDiT,
        "state_dict_converter": wan_video_dit_from_diffusers,
    },
    {
...
class DeferredWanVideoVAE38(torch.nn.Module):
    """Metadata-only Wan VAE handle that materializes only for encode/decode.

    Cache-backed training consumes already normalized raw VAE latents, but the
    model still needs the VAE geometry constants and checkpoint identity. This
    handle keeps that contract without holding the frozen VAE weights on GPU.
    Evaluation can materialize the exact same VAE lazily and offload it to CPU
    afterwards.
    """
    ...
    def __init__(
        ...
        checkpoint_path: str,
        *,
        runtime_device: str | torch.device,
        torch_dtype: torch.dtype,
    ) -> None:
        super().__init__()
        ...
        # Keep the lazily loaded frozen codec outside Module registration.
        # DeepSpeed/Accelerate must never treat it as part of the training
        # module or serialize it into optimizer/trainer state after evaluation.
        object.__setattr__(self, "_materialized_vae", None)

    @property
    def is_materialized(self) -> bool:
        return self._materialized_vae is not None
    ...
    def _ensure_materialized(self) -> WanVideoVAE38:
        vae = self._materialized_vae
        if vae is None:
            logger.info(
                "Materializing deferred Wan VAE for encode/decode from %s",
            ...
            object.__setattr__(self, "_materialized_vae", vae)
            return vae

        parameter = next(vae.parameters(), None)
        if parameter is not None and (
            parameter.device != self.runtime_device
            or parameter.dtype != self.runtime_dtype
        ):
            vae.to(device=self.runtime_device, dtype=self.runtime_dtype)
        vae.eval()
        return vae

    def offload(self) -> bool:
        """Move a materialized evaluation VAE off GPU; return whether it existed."""

        vae = self._materialized_vae
        if vae is None:
            return False
        vae.to(device="cpu", dtype=self.runtime_dtype)
        logger.info("Offloaded deferred Wan VAE to CPU after evaluation.")
        return True

    def encode(self, *args, **kwargs):
        return self._ensure_materialized().encode(*args, **kwargs)

    def decode(self, *args, **kwargs):
        return self._ensure_materialized().decode(*args, **kwargs)
...
def load_wan22_ti2v_5b_components(
    ...
    if defer_vae_load:
        vae = DeferredWanVideoVAE38(
            str(vae_config.path),
            runtime_device=device,
            torch_dtype=torch_dtype,
        )
        logger.info(
            "Deferred Wan VAE weight loading; cache-backed training keeps only "
            "the VAE geometry and checkpoint contract resident."
        )
    else:
        vae = _load_registered_model(
            vae_config.path,
