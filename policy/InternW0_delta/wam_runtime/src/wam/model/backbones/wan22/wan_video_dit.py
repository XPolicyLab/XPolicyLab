...
def precompute_rope_freq_bases(dim: int, theta: float = 10000.0) -> torch.Tensor:
    if dim <= 0 or dim % 2 != 0:
        raise ValueError(f"RoPE axis dim must be a positive even integer, got {dim}.")
    return 1.0 / (
        theta
        ** (
...
def precompute_rope_freq_bases_3d(
    dim: int, theta: float = 10000.0
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    temporal_dim = dim - 2 * (dim // 3)
    spatial_dim = dim // 3
    return (
        precompute_rope_freq_bases(temporal_dim, theta),
        precompute_rope_freq_bases(spatial_dim, theta),
        precompute_rope_freq_bases(spatial_dim, theta),
    )


def build_3d_rope_freqs(
    freq_bases_3d: tuple[torch.Tensor, torch.Tensor, torch.Tensor],
    t_ids: torch.Tensor,
    h_ids: torch.Tensor,
    w_ids: torch.Tensor,
    *,
    ...
    """Build per-token 3D RoPE frequencies.

    Accepts either shared position ids [S] or batched ids [B, S]. The returned
    tensor is shaped [S, 1, D] for shared ids and [B, S, 1, D] for batched ids,
    both of which broadcast correctly in `rope_apply`.
    """
    if t_ids.shape != h_ids.shape or t_ids.shape != w_ids.shape:
        raise ValueError(
            "3D RoPE id shapes must match, got "
            f"t={tuple(t_ids.shape)} h={tuple(h_ids.shape)} w={tuple(w_ids.shape)}"
        )
    if t_ids.ndim not in (1, 2):
        raise ValueError(f"3D RoPE ids must be [S] or [B,S], got {tuple(t_ids.shape)}")
    if device is None:
        device = t_ids.device

    t_ids = t_ids.to(device=device, dtype=torch.float64)
    h_ids = h_ids.to(device=device, dtype=torch.float64)
    w_ids = w_ids.to(device=device, dtype=torch.float64)

    def _axis_freqs(
        position_ids: torch.Tensor, freq_bases: torch.Tensor
    ) -> torch.Tensor:
        phases = position_ids.unsqueeze(-1) * freq_bases.to(device=device)
        return torch.polar(torch.ones_like(phases), phases)

    freqs = torch.cat(
        [
            _axis_freqs(t_ids, freq_bases_3d[0]),
            _axis_freqs(h_ids, freq_bases_3d[1]),
            _axis_freqs(w_ids, freq_bases_3d[2]),
        ],
        dim=-1,
    )
    if freqs.ndim == 2:
...
class CrossAttention(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.hidden_dim = hidden_dim
        self.context_dim = int(hidden_dim if context_dim is None else context_dim)
        self.num_heads = num_heads
        self.attn_head_dim = attn_head_dim
        self.attn_hidden_dim = self.num_heads * self.attn_head_dim

        self.q = nn.Linear(hidden_dim, self.attn_hidden_dim)
        self.k = nn.Linear(self.context_dim, self.attn_hidden_dim)
        self.v = nn.Linear(self.context_dim, self.attn_hidden_dim)
        self.o = nn.Linear(self.attn_hidden_dim, hidden_dim)
        self.norm_q = RMSNorm(self.attn_hidden_dim, eps=eps)
        self.norm_k = RMSNorm(self.attn_hidden_dim, eps=eps)
...
class DiTBlock(nn.Module):
    ...
    def _modulation_parts(self, t_mod: torch.Tensor):
        has_seq = t_mod.ndim == 4
        chunk_dim = 2 if has_seq else 1
        parts = (self.modulation.to(dtype=t_mod.dtype, device=t_mod.device) + t_mod).chunk(
            6,
            dim=chunk_dim,
        )
        if not has_seq:
    ...
    def forward(
        ...
        *,
        modulation_only: bool = False,
    ):
        if t_mod is None:
            raise ValueError("`t_mod` is required for DiTBlock.forward().")
        if modulation_only:
            return self._modulation_parts(t_mod)
        if x is None or context is None or freqs is None:
            raise ValueError("`x`, `context`, and `freqs` are required for a full DiTBlock forward.")
        if context_mask is not None and context_mask.dim() == 3:
            context_mask = context_mask.unsqueeze(1) # (B, 1, seq_len, context_len), 1 for heads
        # msa: multi-head self-attention  mlp: multi-layer perceptron
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self._modulation_parts(t_mod)
...
class WanVideoDiT(torch.nn.Module):
    def __init__(
        ...
        assert not has_image_input
        assert not require_clip_embedding
        assert not require_vae_embedding and fuse_vae_embedding_in_latents, "Only support fusing vae embedding in latents"
        ...
        rope_freq_bases = precompute_rope_freq_bases_3d(attn_head_dim)
        self.register_buffer(
            "rope_temporal_freq_bases", rope_freq_bases[0], persistent=False
        )
        self.register_buffer(
            "rope_height_freq_bases", rope_freq_bases[1], persistent=False
        )
        self.register_buffer(
            "rope_width_freq_bases", rope_freq_bases[2], persistent=False
        )
        if has_ref_conv:
        ...
        self.future_delta_patch_dim = 0
        self.register_parameter("future_delta_queries", None)
        self.future_delta_head: Optional[nn.Module] = None

    @property
    def rope_freq_bases_3d(
        self,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        return (
            self.rope_temporal_freq_bases,
            self.rope_height_freq_bases,
            self.rope_width_freq_bases,
        )

    def configure_future_delta_queries(
        self,
        *,
        num_tokens: int,
        patch_dim: int,
        num_future_delta_frames: int,
    ) -> None:
        """Configure spatial Future Delta queries in the video expert."""
        num_tokens = int(num_tokens)
        patch_dim = int(patch_dim)
        num_future_delta_frames = int(num_future_delta_frames)
        ref = next(self.parameters())
        self.future_delta_queries = nn.Parameter(
            torch.randn(
                1,
                num_tokens,
                self.hidden_dim,
        ...
        self.future_delta_head = nn.Sequential(
            nn.LayerNorm(self.hidden_dim, eps=self.eps),
            nn.Linear(
                self.hidden_dim,
                patch_dim * num_future_delta_frames,
            ),
        ).to(device=ref.device, dtype=ref.dtype)
        self.future_delta_num_tokens = num_tokens
        self.future_delta_num_frames = num_future_delta_frames
        self.future_delta_patch_dim = patch_dim

    def append_future_delta_queries(
        self,
        pre_state: Dict[str, Any],
        *,
        current_frame_index: int,
    ) -> None:
        """Append one current-sized query map for the full future trajectory."""
        tokens = pre_state["tokens"]
        batch_size = int(tokens.shape[0])
        tokens_per_frame = int(pre_state["meta"]["tokens_per_frame"])
        num_tokens = int(self.future_delta_num_tokens)
        if num_tokens != tokens_per_frame:
            raise ValueError(
                "Video Future Delta query count must equal one video frame: "
                f"{num_tokens} vs {tokens_per_frame}."
            )
        real_video_seq_len = int(tokens.shape[1])
        current_start = int(current_frame_index) * tokens_per_frame
        current_end = current_start + tokens_per_frame

        queries = self.future_delta_queries.to(
            device=tokens.device, dtype=tokens.dtype
        ).expand(batch_size, -1, -1)
        pre_state["tokens"] = torch.cat([tokens, queries], dim=1)

        _, grid_h, grid_w = pre_state["meta"]["grid_size"]
        midpoint = float(current_frame_index) + 0.5
        t_ids = torch.full(
            (num_tokens,), midpoint, dtype=torch.float64, device=tokens.device
        )
        h_ids = (
        ...
        query_freqs = build_3d_rope_freqs(
            self.rope_freq_bases_3d,
            t_ids,
            h_ids,
            w_ids,
            device=tokens.device,
        ).to(dtype=pre_state["freqs"].dtype)
        if pre_state["freqs"].ndim == 4:
            query_freqs = query_freqs.unsqueeze(0).expand(
                batch_size, -1, -1, -1
            )
        pre_state["freqs"] = torch.cat(
            [pre_state["freqs"], query_freqs], dim=-3
        )

        zero_timestep = torch.zeros(
            (batch_size,), dtype=pre_state["t"].dtype, device=tokens.device
        )
        query_t = self.time_embedding(
            sinusoidal_embedding_1d(self.freq_dim, zero_timestep)
        )
        query_t_mod = self.time_projection(query_t).unflatten(
            1, (6, self.hidden_dim)
        )
        query_t_mod = query_t_mod[:, None].expand(
            -1, num_tokens, -1, -1
        )
        pre_state["t_mod"] = torch.cat(
            [pre_state["t_mod"], query_t_mod], dim=1
        ).contiguous()

        context_mask = pre_state["context_mask"]
        query_context_mask = context_mask[:, current_start:current_end]
        pre_state["context_mask"] = torch.cat(
            [context_mask, query_context_mask], dim=1
        )
        pre_state["meta"]["real_video_seq_len"] = real_video_seq_len
        pre_state["meta"]["num_future_delta_tokens"] = num_tokens
    ...
    def predict_future_delta(
        ...
        start = int(pre_state["meta"]["real_video_seq_len"])
        end = start + int(pre_state["meta"]["num_future_delta_tokens"])
        prediction = self.future_delta_head(tokens[:, start:end])
        batch_size, num_tokens, _ = prediction.shape
        return prediction.view(
            batch_size,
            num_tokens,
            self.future_delta_num_frames,
            self.future_delta_patch_dim,
        ).permute(0, 2, 1, 3).reshape(
            batch_size,
            self.future_delta_num_frames * num_tokens,
            self.future_delta_patch_dim,
        )
    ...
    def _validate_forward_inputs(
        ...
        if timestep.ndim not in (1, 2):
            raise ValueError(
                "`timestep` must be [B], [1], or per-future-frame [B,F-1], "
        ...
        if timestep.ndim == 2 and int(timestep.shape[1]) != num_latent_frames - 1:
            raise ValueError(
                "Per-frame video timesteps must cover every noisy latent frame: "
                f"got {tuple(timestep.shape)}, expected "
                f"[{batch_size},{num_latent_frames - 1}]."
            )
    ...
    def pre_dit(
        ...
        if self.seperated_timestep and fuse_vae_embedding_in_latents:
            ...
            if timestep.ndim == 1:
                frame_timesteps = timestep[:, None].expand(
                    batch_size, int(x.shape[2]) - 1
                )
            else:
            ...
            token_timesteps = frame_timesteps[:, :, None].expand(
                batch_size, int(x.shape[2]), tokens_per_frame
            )
        ...
        if frame_ids is None:
            frame_grid = (
                torch.arange(f, device=x_tokens.device)
                .view(f, 1, 1)
                .expand(f, h, w)
                .reshape(f * h * w)
            )
            h_grid = (
                torch.arange(h, device=x_tokens.device)
                .view(1, h, 1)
                .expand(f, h, w)
                .reshape(f * h * w)
            )
            w_grid = (
                torch.arange(w, device=x_tokens.device)
                .view(1, 1, w)
                .expand(f, h, w)
                .reshape(f * h * w)
            )
            freqs = build_3d_rope_freqs(
                self.rope_freq_bases_3d,
                frame_grid,
                h_grid,
                w_grid,
            ...
            frame_ids = frame_ids.to(device=x_tokens.device, dtype=torch.long)
            if frame_ids.ndim == 1:
                if int(frame_ids.shape[0]) != int(f):
                    raise ValueError(f"`frame_ids` must have length {f}, got {tuple(frame_ids.shape)}")
                frame_ids = frame_ids.unsqueeze(0).expand(batch_size, -1)
            elif frame_ids.ndim == 2:
                if tuple(frame_ids.shape) != (batch_size, f):
                    raise ValueError(
                        f"`frame_ids` must be [B,{f}] for batched ids, got {tuple(frame_ids.shape)}"
                    )
            else:
                raise ValueError(f"`frame_ids` must be [F] or [B,F], got {tuple(frame_ids.shape)}")
            frame_grid = frame_ids.view(batch_size, f, 1, 1).expand(batch_size, f, h, w).reshape(batch_size, f * h * w)
            h_grid = torch.arange(h, device=x_tokens.device).view(1, 1, h, 1).expand(batch_size, f, h, w).reshape(batch_size, f * h * w)
            w_grid = torch.arange(w, device=x_tokens.device).view(1, 1, 1, w).expand(batch_size, f, h, w).reshape(batch_size, f * h * w)
            freqs = build_3d_rope_freqs(
                self.rope_freq_bases_3d,
                frame_grid,
                h_grid,
    ...
    def post_dit(self, x_tokens: torch.Tensor, pre_state: Dict[str, Any]) -> torch.Tensor:
        ...
        real_video_seq_len = int(
            pre_state["meta"].get("real_video_seq_len", f * h * w)
        )
        x_tokens = x_tokens[:, :real_video_seq_len]
