...
class WanContinuousFlowMatchScheduler:
    ...
    def add_noise(
        ...
        *,
        ...
        if time_dim is not None and sigma.ndim > 1:
            time_dim = int(time_dim)
            if time_dim < 0:
                time_dim += original_samples.ndim
            if time_dim <= 0 or time_dim >= original_samples.ndim:
                raise ValueError(
                    f"`time_dim` must point to a non-batch sample axis, got {time_dim} "
                    f"for samples with ndim={original_samples.ndim}."
                )
            if sigma.ndim != 2:
                raise ValueError(
                    "Per-time noise expects `timestep` shape [B,T], "
                    f"got {tuple(timestep.shape)}."
                )
            expected = (int(original_samples.shape[0]), int(original_samples.shape[time_dim]))
            if tuple(sigma.shape) != expected:
                raise ValueError(
                    "Per-time timestep shape mismatch: "
                    f"got {tuple(sigma.shape)}, expected {expected} for samples {tuple(original_samples.shape)}."
                )
            view_shape = [1] * original_samples.ndim
            view_shape[0] = int(sigma.shape[0])
            view_shape[time_dim] = int(sigma.shape[1])
