...
MEX_EVAL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
...
bash "${MEX_EVAL_DIR}/setup_eval_policy_server.sh" \
...
bash "${MEX_EVAL_DIR}/setup_eval_env_client.sh" \
    "${bench_name}" "${task_name}" "${ckpt_name}" "${env_cfg_type}" \
