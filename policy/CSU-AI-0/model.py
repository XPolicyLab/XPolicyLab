...
def _resolve_pi05_model_root(model_cfg: dict[str, Any]) -> Path:
    ...
    if roots:
        return roots[0]
    raise ValueError("ckpt_name, model_path, or checkpoint_path is required for CSU-AI-0.")
...
class Model(ModelTemplate):
    ...
    def get_model(self, model_cfg: dict[str, Any]):
        train_config_name = model_cfg.get("train_config_name") or "pi05sf_jax_robodojo_v21_offcache"
        repo_id = model_cfg.get("repo_id") or "RoboDojo_lerobot_v21_video"
        model_root = _resolve_pi05_model_root(model_cfg)

        print(f"[CSU-AI-0] Resolving train config: {train_config_name}", flush=True)
        config = _config.get_config(train_config_name)
        norm_stats = None
        if repo_id is not None:
            print(f"[CSU-AI-0] Loading norm stats from {model_root / 'assets' / str(repo_id)}", flush=True)
            norm_stats = _normalize.load(model_root / "assets" / str(repo_id))

        print(f"[CSU-AI-0] Loading checkpoint from {model_root} (may take several minutes)...", flush=True)
        policy = _policy_config.create_trained_policy(config, str(model_root), norm_stats=norm_stats)
        print("[CSU-AI-0] Policy checkpoint loaded.", flush=True)
        return policy
