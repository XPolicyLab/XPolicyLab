...
def decode_and_resize_images(image_bytes_array, size=256):
    ...
    for img_bytes in image_bytes_array:
        img_pil = decode_image_bit(img_bytes)
...
def process_one_episode(input_path, output_path, episode_idx, instruction_dir, resize_size=256, action_type="joint"):
    with h5py.File(input_path, "r") as f:
        ...
        head = decode_and_resize_images(f["vision/cam_head/colors"][()], size=resize_size)
        left = decode_and_resize_images(f["vision/cam_left_wrist/colors"][()], size=resize_size)
        right = decode_and_resize_images(f["vision/cam_right_wrist/colors"][()], size=resize_size)
...
def main(args):
    ...
    all_eps = sorted(glob(os.path.join(load_data_dir, "episode_*.hdf5")))[:expert_data_num]
    random.seed(42)
    random.shuffle(all_eps)

    n_val = int(len(all_eps) * percent_val)
    ...
    for split_name, split_eps in [("train", train_eps), ("val", val_eps)]:
        ...
        for i, ep in enumerate(tqdm(split_eps, desc=f"Processing {split_name}")):
            ...
            try:
                process_one_episode(ep, out_path, i, instruction_dir, resize_size=resize_size, action_type=action_type)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--img_resize_size", type=int, default=256,
                        help="Final size for RGB images")
    parser.add_argument("--out_base_dir", type=str, default="./processed_data",
                        help="Output dir for processed OpenVLA-compatible dataset")
