...
# Modified by the Rex-M1-preview authors (2026): past-reconstruction window bookkeeping
# (ptp_length), inactive in the released checkpoint.
import math
import random
...
class xr1(nn.Module):
    def __init__(
        ...
        async_train=True,
        ptp_length=0,
        ptp_loss_coefficient=1.0,
    ):
        super().__init__()
        self.state_shape = (1, 60)
        self.action_shape = (30, 60)
        self.ptp_length = int(ptp_length)
        self.ptp_loss_coefficient = float(ptp_loss_coefficient)
    ...
    def _random_mask_prefix(self, mask, prefix_length, state_length, keep_last=2, ptp_length=0):
        if prefix_length <= keep_last:
            return mask
        action_start = 1 + state_length + ptp_length  # skip sink, state and the past segment
    ...
    def dit_forward(
        ...
        if prefix_length:
            output[:, ptp_length : ptp_length + prefix_length] = 0.0  # the known future prefix sits after the past segment
        return output
    ...
    def forward(self, batch, return_loss=False):
        ...
        past_action = batch.pop("past_action", None)
        past_action_mask = batch.pop("past_action_mask", None)
        ptp = self.ptp_length
        if ptp:
            if past_action is None or past_action_mask is None:
                raise ValueError("ptp_length > 0 requires past_action / past_action_mask in the batch")
            if past_action.shape[1] != ptp:
                raise ValueError(f"past_action has {past_action.shape[1]} rows, expected ptp_length={ptp}")
            action = torch.cat([past_action.to(action.dtype), action], dim=1)
            action_mask = torch.cat([past_action_mask.to(action_mask.dtype), action_mask], dim=1)
        if self.training:
        ...
        batch_size, action_length, _ = action.shape  # total rows = ptp + future
        future_length = action_length - ptp
        state_length = state.shape[1]
        query_length = 1 + state_length + action_length
        prefix = action[:, ptp : ptp + prefix_length]  # the known prefix is the start of the FUTURE chunk
        ...
        if future_length > prefix_length:
            dit_position_ids[:, :, -(future_length - prefix_length) :] += 10
        ...
        if self.training and prefix_length > 2:
            causal_mask = self._random_mask_prefix(causal_mask, prefix_length, state_length, ptp_length=ptp)
        ...
        def with_prefix(noisy):
            return torch.cat([noisy[:, :ptp], prefix, noisy[:, ptp + prefix_length :]], dim=1)
        ...
        if self.training:
            ...
            target = action - noise
            pred = self.dit_forward(with_prefix(noisy_action), timestep, **kwargs)
            if prefix_length:
                prefix_pred = self._generate(with_prefix(noise), kwargs)
                weight = (prefix_pred - action).abs()
            else:
                weight = torch.ones_like(pred)
            loss_mask = action_mask.clone()
            if prefix_length:
                loss_mask[:, ptp : ptp + prefix_length] = 0
        else:
            return self._generate(with_prefix(noise), kwargs)

        if not return_loss:
            return pred

        loss_mse, loss_freq, loss_ptp = self.compute_flow_loss(
            pred, target, loss_mask, weight, ptp_length=ptp, future_start=ptp + prefix_length
        )
        ...
        if ptp:
            loss = loss + 0.5 * self.ptp_loss_coefficient * loss_ptp
        out = {
            "loss": loss,
            "loss_mse": loss_mse,
            "loss_freq": loss_freq,
        ...
        if ptp:
            out["loss_ptp"] = loss_ptp
        return out

    @staticmethod
    def _normalized_weight(weight, action_mask):
        with torch.no_grad():
            weight = weight.clone()
    ...
    def compute_flow_loss(self, pred, target, action_mask, weight, ptp_length=0, future_start=0):
        """Returns (loss_mse, loss_freq, loss_ptp).

        ptp_length: rows at the start of the window scored as loss_ptp (0 disables it).
        future_start: rows of the future chunk excluded from loss_mse / loss_freq.
        """
        ...
        loss_ptp = zero
        if ptp_length:
            pm = action_mask[:, :ptp_length]
            if torch.any(pm):
                pw = self._normalized_weight(weight[:, :ptp_length], pm)
                loss_ptp = (F.mse_loss(pred[:, :ptp_length], target[:, :ptp_length], reduction="none") * pw)[pm].mean()
        if future_start:
            pred, target = pred[:, future_start:], target[:, future_start:]
            action_mask, weight = action_mask[:, future_start:], weight[:, future_start:]
        if not torch.any(action_mask):
            return zero, zero, loss_ptp
        weight = self._normalized_weight(weight, action_mask)
        ...
        if not torch.any(valid_batch):
            return loss_mse, freq.sum() * 0.0, loss_ptp
        ...
        return loss_mse, loss_freq, loss_ptp
