...
# Modified by the Rex-M1-preview authors (2026): compute_flow_loss returns a third value
# (loss_ptp), so the two unpack sites below take three.
import numpy as np
import torch
...
def test_flow_loss_with_empty_mask_is_finite():
    ...
    loss_mse, loss_freq, _ = model.compute_flow_loss(pred, target, mask, torch.ones_like(pred))
