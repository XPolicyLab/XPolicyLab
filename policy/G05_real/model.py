...
class Model(ModelTemplate):
    def __init__(self, model_cfg):
        ...
        if self.env_cfg_type not in REAL_EMBODIMENT_BY_ENV:
            raise ValueError(
                f"unsupported RoboDojo real env_cfg_type={self.env_cfg_type!r}; "
                f"expected one of {sorted(REAL_EMBODIMENT_BY_ENV)}"
            )
        expected_embodiment = REAL_EMBODIMENT_BY_ENV[self.env_cfg_type]
        self.embodiment_type = str(model_cfg.get("eval_embodiment") or "")
        if self.embodiment_type != expected_embodiment:
            raise ValueError(
                f"env/checkpoint embodiment mismatch: env_cfg_type={self.env_cfg_type!r} "
                f"requires eval_embodiment={expected_embodiment!r}, "
                f"got {self.embodiment_type!r}"
            )
        self.action_steps = int(model_cfg.get("action_steps") or 16)
        self.default_frequency = float(model_cfg.get("frequency") or 25)
        if self.default_frequency != 25.0:
            raise ValueError(
                f"RoboDojo real policies must run at 25 Hz, got {self.default_frequency}"
            )
        ...
        checkpoint_embodiment = str(cfg.data.get("embodiment") or "")
        if checkpoint_embodiment != self.embodiment_type:
            raise ValueError(
                f"checkpoint was trained for {checkpoint_embodiment!r}, but server "
                f"was configured for {self.embodiment_type!r}"
            )
        self._validate_stats_embodiment(run_dir)
    ...
    def _validate_stats_embodiment(self, run_dir: Path) -> None:
        required = [Path(run_dir) / "hf_processor", Path(run_dir) / "action_tokenizer_hf"]
        missing = [str(path) for path in required if not path.is_dir()]
        if missing:
            raise FileNotFoundError(
                "checkpoint bundle is missing processor assets:\n  " + "\n  ".join(missing)
            )
        stats_path = Path(run_dir) / "dataset_stats.json"
        if not stats_path.is_file():
            raise FileNotFoundError(
                f"checkpoint bundle is missing per-embodiment stats: {stats_path}"
            )
        payload = json.loads(stats_path.read_text())
        if set(payload) != {self.embodiment_type}:
            raise ValueError(
                f"dataset_stats.json must contain exactly {self.embodiment_type!r}; "
    ...
    def _resolve_ckpt_path(self, model_cfg) -> Path:
        cfg = dict(model_cfg)
        if os.environ.get("G05_CKPT_PATH"):
            cfg["ckpt_path"] = os.environ["G05_CKPT_PATH"]
    ...
    def _to_g05_raw_obs(self, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        if frequency != self.default_frequency:
            raise ValueError(
                f"RoboDojo real observation frequency must be "
                f"{self.default_frequency:g} Hz, got {frequency:g} Hz"
            )
        return {
            "images": images,
            "state": state,
...
def _as_chw_uint8(value) -> np.ndarray:
    if not isinstance(value, np.ndarray):
        raise TypeError(
            "XPolicyLab policy server must provide decoded RGB numpy arrays; "
