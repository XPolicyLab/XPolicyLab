...
def _pad_last_dim(values: np.ndarray, dim: int) -> np.ndarray:
...
def _pad_stat_values(values, dim: int) -> list[float]:
    values = np.asarray(values, dtype=np.float32).reshape(-1)
    padded = np.zeros(dim, dtype=np.float32)
    copy_dim = min(len(values), dim)
    if copy_dim:
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        self._v3_episode_meta_cache: dict[int, dict] | None = None
        self._v3_data_file_cache: tuple[Path, pd.DataFrame] | None = None
    ...
    def _get_v3_episode_meta(self) -> dict[int, dict]:
        if self._v3_episode_meta_cache is not None:
            return self._v3_episode_meta_cache
        episode_files = sorted((self.dataset_path / "meta" / "episodes").glob("chunk-*/*.parquet"))
        if not episode_files:
            self._v3_episode_meta_cache = {}
            return self._v3_episode_meta_cache
            ...
            if not modality_meta_path.exists() and self.is_lerobot_v3 and self.tag == EmbodimentTag.AGIBOT:
                modality_meta = {
                    ...
                    "video": {
                        ...
                        for key, value in AGIBOT_V3_VIDEO_MAPPING.items()
                    },
                    "annotation": {
        ...
        if self.is_lerobot_v3 and self.tag == EmbodimentTag.AGIBOT:
            ...
            if stats:
                ...
                try:
                    ...
                    print(f"Failed to save LeRobot v3 relative stats to {save_path}: {e}")
            return stats
    ...
    def _calculate_v3_relative_stats(
        self,
        action_keys_to_process: list[str],
    ) -> dict[str, DatasetStatisticalValues]:
        """Calculate relative action stats directly from LeRobot v3 parquet shards."""
        print(f"Calculating LeRobot v3 relative stats for: {action_keys_to_process}")
        samples: dict[str, list[np.ndarray]] = {key: [] for key in action_keys_to_process}
    ...
    def _get_trajectories(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        if self.is_lerobot_v3:
            episode_meta = self._get_v3_episode_meta()
            trajectory_ids = sorted(episode_meta)
            trajectory_lengths = [int(episode_meta[ep]["length"]) for ep in trajectory_ids]
    ...
    def _get_tasks(self) -> pd.DataFrame:
        ...
        v3_tasks_path = self.dataset_path / "meta" / "tasks.parquet"
        if self.is_lerobot_v3 and v3_tasks_path.exists():
            df = pd.read_parquet(v3_tasks_path)
            rows = [
    ...
    def get_parquet_path(self, trajectory_id: int) -> Path:
        ...
        if self.is_lerobot_v3:
            episode = self._get_v3_episode_meta()[int(trajectory_id)]
    ...
    def get_trajectory_data(self, trajectory_id: int) -> pd.DataFrame:
        ...
        if self.is_lerobot_v3:
            if self._v3_data_file_cache is not None and self._v3_data_file_cache[0] == parquet_path:
                parquet_df = self._v3_data_file_cache[1]
            else:
                parquet_df = pd.read_parquet(parquet_path)
                self._v3_data_file_cache = (parquet_path, parquet_df)
            traj_data = parquet_df.loc[parquet_df["episode_index"] == int(trajectory_id)].copy()
            traj_data = traj_data.sort_values("frame_index").reset_index(drop=True)
            episode = self._get_v3_episode_meta()[int(trajectory_id)]
            offset = self._get_v3_video_time_offset(episode)
            if offset:
                traj_data["timestamp"] = traj_data["timestamp"].astype(float) + offset
    ...
    def _get_v3_video_time_offset(self, episode: dict) -> float:
        for video_key in AGIBOT_V3_VIDEO_MAPPING.values():
            from_key = f"videos/{video_key}/from_timestamp"
            if from_key in episode and not pd.isna(episode[from_key]):
                return float(episode[from_key])
        return 0.0
    ...
    def get_video_path(self, trajectory_id: int, key: str) -> Path:
        ...
        if self.is_lerobot_v3:
            ...
            video_filename = self.video_path_pattern.format(
                ...
                chunk_index=int(episode[f"videos/{original_key}/chunk_index"]),
                file_index=int(episode[f"videos/{original_key}/file_index"]),
            )
