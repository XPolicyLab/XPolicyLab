...
class Model(ModelTemplate):
    def __init__(self, model_cfg):
        ...
        self.inference_batch_size = max(
            1,
            int(
                os.environ.get("ROBODOJO_G05_INFERENCE_BATCH_SIZE")
                or model_cfg.get("inference_batch_size")
                or "8"
            ),
        )
        ...
        required_g05_paths = (
            g05_root / "pyproject.toml",
            g05_root / "scripts" / "serve_policy.py",
            g05_root / "src" / "g05" / "models" / "g05" / "inferencer.py",
        )
        missing_g05_paths = [path for path in required_g05_paths if not path.is_file()]
        if missing_g05_paths:
            missing = ", ".join(str(path) for path in missing_g05_paths)
            raise FileNotFoundError(
                "CSU-AI-05's evaluated G05 runtime is incomplete. "
                f"Missing: {missing}. Set G05_ROOT only to use another compatible checkout."
            )
        self.g05_root = g05_root
        self.vision_attn_backend = str(
            os.environ.get("G05_VISION_ATTN_BACKEND")
            or model_cfg.get("vision_attn_backend")
            or "sdpa"
        ).strip().lower()
        if self.vision_attn_backend not in {"auto", "fa4", "fa2", "sdpa"}:
            raise ValueError(
                "vision_attn_backend must be one of auto/fa4/fa2/sdpa, "
                f"got {self.vision_attn_backend!r}"
            )
        os.environ["G05_VISION_ATTN_BACKEND"] = self.vision_attn_backend
        ...
        requested_action_source = str(
            os.environ.get("ROBODOJO_G05_ACTION_SOURCE")
            or model_cfg.get("action_source")
            or "auto"
        )
        hydra_overrides = [
            str(item)
        ...
        self.runtime_assets = _prepare_g05_runtime_env(ckpt_path, run_dir)
    ...
    def _resolve_robot_action_dim_info(self) -> dict[str, list[int]]:
        try:
            ...
            # A standalone XPolicyLab checkout has no sibling env_cfg/ tree,
            # while the current adapter contract registers robot dimensions in
            # XPolicyLab/utils/robot/_robot_info.json. Use that same registry
            # when the benchmark-specific helper cannot resolve its env files.
            registry_path = (
            ...
            try:
                ...
                registered = registry[self.env_cfg_type]
                dims = {
                    "arm_dim": [int(value) for value in registered["arm_dim"]],
                    "ee_dim": [int(value) for value in registered["ee_dim"]],
                }
                if not dims["arm_dim"] or not dims["ee_dim"]:
                    raise ValueError("registered dimensions must not be empty")
                if any(value <= 0 for values in dims.values() for value in values):
                    raise ValueError("registered dimensions must be positive")
                return dims
            except Exception as registry_exc:
                raise KeyError(
                    f"Robot {self.env_cfg_type!r} is not registered in "
                    f"{registry_path}"
                ) from registry_exc
    ...
    def _build_raw_state(self, packed_state: np.ndarray) -> dict[str, np.ndarray]:
        ...
        arm_dims = [int(dim) for dim in self.robot_action_dim_info["arm_dim"]]
        ee_dims = [int(dim) for dim in self.robot_action_dim_info["ee_dim"]]
        if len(arm_dims) != 2 or len(ee_dims) != 2:
            raise ValueError(
                f"G05 requires two-arm dimensions, got arm_dim={arm_dims}, ee_dim={ee_dims}"
            )
        expected_dim = sum(arm_dims) + sum(ee_dims)
        if packed_state.shape[-1] != expected_dim:
            raise ValueError(
                f"Packed state dimension {packed_state.shape[-1]} does not match "
                f"registered robot dimension {expected_dim}"
            )

        cursor = 0
        left_arm = packed_state[cursor : cursor + arm_dims[0]]
        cursor += arm_dims[0]
        left_gripper = packed_state[cursor : cursor + ee_dims[0]]
        cursor += ee_dims[0]
        right_arm = packed_state[cursor : cursor + arm_dims[1]]
        cursor += arm_dims[1]
        right_gripper = packed_state[cursor : cursor + ee_dims[1]]
...
def _prepare_g05_runtime_env(ckpt_path: Path, run_dir: Path) -> dict[str, Path]:
    """Resolve the self-contained CSU-AI-05 sidecars required by the saved Hydra config."""
    ckpt_path = Path(ckpt_path).expanduser().resolve()
    bundle_root = Path(run_dir).expanduser().resolve()
    if not ckpt_path.is_file():
        raise FileNotFoundError(f"G0.5 checkpoint not found: {ckpt_path}")

    def resolve_asset(env_name: str, candidates: tuple[Path, ...], *, directory: bool) -> Path:
        ...
        if raw:
            ...
            if path is None:
                ...
                raise FileNotFoundError(
                    f"Missing CSU-AI-05 runtime {kind} for {env_name}. Expected one of: {expected}. "
                    f"Add it to the checkpoint bundle or set {env_name} explicitly."
                )
        valid = path.is_dir() if directory else path.is_file()
        if not valid:
            kind = "directory" if directory else "file"
            raise FileNotFoundError(f"{env_name} must point to an existing {kind}: {path}")
        os.environ[env_name] = str(path)
        return path

    action_tokenizer = resolve_asset(
        "G05_ACTION_TOKENIZER_PATH",
        (bundle_root / "action_tokenizer.pt",),
        directory=False,
    )
    dataset_stats = resolve_asset(
        "G05_MIXED_STATS_PATH",
        (bundle_root / "dataset_stats.json",),
        directory=False,
    )
    processor = resolve_asset(
        "G05_HF_PROCESSOR_PATH",
        (bundle_root / "processor", bundle_root / "hf_processor"),
        directory=True,
    )

    # The saved config contains nullable oc.env interpolations. G05's standalone
    # config loader treats a YAML null fallback as missing, so provide every
    # inference-side value explicitly before resolving the Hydra configuration.
    os.environ["G05_CKPT_PATH"] = str(ckpt_path)
    os.environ["G05_BASE_CKPT"] = str(ckpt_path)
    os.environ["G05_RESUME_CKPT"] = str(ckpt_path)
    os.environ["G05_ACTION_TOKENIZER"] = str(action_tokenizer)
    os.environ["G05_STATS_PATH"] = str(dataset_stats)
    os.environ["G05_HF_PROCESSOR"] = str(processor)
    os.environ.setdefault("ROBODOJO_LEROBOT_V30_ROOT", str(bundle_root))
    os.environ.setdefault("G05_OWN_DATA_ROOT", str(bundle_root))
...
def _as_chw_uint8(value) -> np.ndarray:
    if not isinstance(value, np.ndarray):
        raise TypeError(
            "XPolicyLab policy server must decode camera colors before model.update_obs; "
