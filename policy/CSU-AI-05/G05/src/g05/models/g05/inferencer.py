...
class PolicyInferencer:
    ...
    def __init__(
        ...
        processor,
        device: str = "cuda",
        action_source: str = "auto",
    ):
        """Create an inferencer with an explicit action-decoding route.

        The RoboDojo PR82 adapter passes ``action_source`` after applying the
        matching Hydra flags to the policy.  Older GalaxeaVLA checkouts did not
        yet expose this constructor argument, even though their policy routing
        already follows those flags.  Keep the argument here as a validated
        compatibility contract; the policy remains the single source of truth
        for the actual FM/AR forward path.
        """
        action_source = str(action_source).lower()
        if action_source not in {"auto", "fm", "ar", "both"}:
            raise ValueError(f"Unsupported action_source={action_source!r}")
        if action_source == "fm" and not bool(getattr(policy, "continuous_action", False)):
            raise ValueError("action_source='fm' requires policy.continuous_action=true")
        if action_source == "ar" and not bool(getattr(policy, "discrete_action", False)):
            raise ValueError("action_source='ar' requires policy.discrete_action=true")
        self.policy = policy
