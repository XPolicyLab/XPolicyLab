...
_requested_attn_backend = os.environ.get("G05_VISION_ATTN_BACKEND", "auto").strip().lower()
if _requested_attn_backend not in {"auto", "fa4", "fa2", "sdpa"}:
    raise ValueError(
        "G05_VISION_ATTN_BACKEND must be one of auto/fa4/fa2/sdpa, "
        f"got {_requested_attn_backend!r}"
    )

try:
    # The currently packaged FA4 kernels only support SM90+.  Importing them
    # succeeds on Ampere (e.g. A800 / SM80), but the first backward then fails
    # with "Unsupported compute capability".  Skip FA4 up front so Ampere can
    # use FA2 when installed, or the existing PyTorch SDPA fallback otherwise.
    if _requested_attn_backend not in {"auto", "fa4"}:
        raise ImportError(f"FA4 disabled by G05_VISION_ATTN_BACKEND={_requested_attn_backend}")
    if not torch.cuda.is_available() or torch.cuda.get_device_capability()[0] < 9:
        raise ImportError("FlashAttention 4 requires a CUDA device with SM90+")
    ...
    try:
        if _requested_attn_backend not in {"auto", "fa2"}:
            raise ImportError(
                f"FA2 disabled by G05_VISION_ATTN_BACKEND={_requested_attn_backend}"
            )
    ...
    def _attend_spatial(
        ...
        if _flash_attn_varlen is not None:
            ...
            if not _VISION_FLASH_ATTN_WARNED:
                logger.warning(
                    "Vision attention uses SDPA fallback "
                    f"(requested_backend={_requested_attn_backend}). "
