# Modified or added for the CSU-AI-05 RoboDojo evaluation.
import torch
import logging
import itertools
...
def load_model_from_checkpoint(
    ...
    # Split checkpoint format: the rolling training-state file intentionally
    # excludes model tensors and points at an immutable model-only snapshot.
    # Resolve and merge it transparently so existing resume code keeps working.
    if full_ckpt.get(state_dict_key) is None and full_ckpt.get("model_checkpoint"):
        model_ckpt_path = Path(full_ckpt["model_checkpoint"])
        if not model_ckpt_path.is_absolute():
            model_ckpt_path = ckpt_path.parent / model_ckpt_path
        ...
        full_ckpt[state_dict_key] = model_ckpt[state_dict_key]
        if full_ckpt.get("ema_model_state_dict") is None:
            full_ckpt["ema_model_state_dict"] = model_ckpt.get("ema_model_state_dict")
        del model_ckpt
...
def save_training_checkpoint(
    ...
    **extra_state,
    ...
    model_state.update(extra_state)

    if state_latest_only:
        # Keep every inference-capable model snapshot, but only one rolling copy
        # of the large optimizer/scheduler state.  The state references the model
        # snapshot instead of duplicating its tensors.
        torch.save(model_state, path)
        if optimizer is not None:
            state_path = output_dir / "training_state_latest.pt"
            tmp_state_path = output_dir / "training_state_latest.pt.tmp"
            training_state = {
                "step": step,
                "epoch": epoch,
                "batch_idx": batch_idx,
                "model_state_dict": None,
                "model_checkpoint": str(path.relative_to(output_dir)),
            ...
            link_target = state_path
        else:
            # Preserve the last resumable state when the final model-only save runs.
            state_path = output_dir / "training_state_latest.pt"
            link_target = state_path if state_path.exists() else path
    else:
    ...
    if last_pt_path.exists() or last_pt_path.is_symlink():
        last_pt_path.unlink()
    last_pt_path.symlink_to(link_target.relative_to(output_dir))
    return path
