...
图像要求与 XPolicyLab 一致：**640×480 RGB**（由 BGR 转换），训练与部署保持一致。
...
```text
policy/MolmoACT2/data/<dataset_name>-<ckpt_name>-<env_cfg_type>-<expert_data_num>-<action_type>
```
...
1. 从 `demo_env/data/<dataset_name>/<task_name>/<env_cfg>/` 读取 RoboDojo HDF5 轨迹（见 XPolicyLab 根目录 README）。
...
```text
policy/MolmoACT2/checkpoints/<dataset_name>-<ckpt_name>-<env_cfg_type>-<expert_data_num>-<action_type>-<seed>
```
...
| --- | --- |
| `vision/cam_head/color` (BGR) | `observation.images.image` (RGB, 640×480) |
...
`model.py` 需实现 `ModelTemplate` 的 `update_obs` / `get_action` / `reset` 等接口；推理侧注意 **BGR→RGB** 与 **action 归一化/反归一化**（使用 checkpoint 内 `norm_stats.json` 或数据集 quantile stats）。
