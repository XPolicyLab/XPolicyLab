...
### 命名约定（XPolicyLab 4 元组）

处理后数据目录：

```text
policy/MolmoACT2/data/<bench_name>-<ckpt_name>-<env_cfg_type>-<action_type>
```
...
bash process_data.sh RoboDojo stack_bowls arx_x5 joint 50   # 可选 episode 上限
```
...
### 命名约定（XPolicyLab 5 元组 = 4 元组 + seed）

```text
policy/MolmoACT2/checkpoints/<bench_name>-<ckpt_name>-<env_cfg_type>-<action_type>-<seed>
```

### 调用方式

`train.sh` 遵循 XPolicyLab 统一 6 参数：
...
```text
policy/MolmoACT2/checkpoints/RoboDojo-cotrain-arx_x5-joint-0
```
...
`eval.sh` 遵循 XPolicyLab 统一 10 参数。policy 侧使用 `molmoact2/lerobot/.venv`（与训练相同），在 `deploy.yml` 中通过 `policy_uv_env_path: molmoact2/lerobot` 指定；`eval.sh` 第 9 参数传 `uv` 即可。
...
| `expert_data_num` | process_data 可选尾参 | — | 数据处理 episode 上限；train.sh 不接收 |
