...
resolve_checkpoint_path() {
    ...
    if [[ -f "${CKPT_ROOT}/${name}/model.safetensors" || -f "${CKPT_ROOT}/${name}/model.pt" ]]; then
        echo "${CKPT_ROOT}/${name}"
        return 0
    fi
...
if ! checkpoint_path="$(resolve_checkpoint_path "${ckpt_name}")"; then
    ...
    echo -e "\033[31m[SERVER] tried: RISE_CHECKPOINT_PATH, abs dir, ${SCRIPT_DIR}/checkpoints/<name>, ${CKPT_ROOT}/<step>\033[0m" >&2
    exit 1
fi
