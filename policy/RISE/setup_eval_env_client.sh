...
# Load RISE/deploy/sitecustomize.py (socket timeout patch only) in the eval client process.
RISE_CLIENT_ENV=(
    "PYTHONPATH=${RISE_SITECUSTOMIZE_DIR}:${ROOT_DIR}:${PYTHONPATH:-}"
    "RISE_MODEL_CLIENT_TIMEOUT=${RISE_MODEL_CLIENT_TIMEOUT:-600}"
...
if [[ "${eval_env}" == "real" ]]; then
    env "${RISE_CLIENT_ENV[@]}" \
        bash "${UTILS_DIR}/run_real_policy_client.sh" \
            "${eval_batch}" \
            "${eval_env_conda_env}" \
            "${policy_server_port}" \
            "${dataset_name}" \
