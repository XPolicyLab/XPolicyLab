...
ME_DEX_EVAL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
...
XPL_ROOT="$(cd "${ME_DEX_EVAL_DIR}/../.." && pwd)"
...
bash "${ME_DEX_EVAL_DIR}/setup_eval_policy_server.sh" \
...
bash "${ME_DEX_EVAL_DIR}/setup_eval_env_client.sh" \
    "${bench_name}" "${task_name}" "${ckpt_name}" "${env_cfg_type}" \
