...
ME_DEX_POLICY_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
XPL_ROOT="$(cd "${ME_DEX_POLICY_DIR}/../.." && pwd)"
policy_name="$(basename "${ME_DEX_POLICY_DIR}")"
...
if [[ -f "${ME_DEX_POLICY_DIR}/runtime_env.sh" ]]; then
    source "${ME_DEX_POLICY_DIR}/runtime_env.sh"
fi
exec env CUDA_VISIBLE_DEVICES="${policy_gpu_id}" python "${XPL_ROOT}/setup_policy_server.py" \
    --config_path "${ME_DEX_POLICY_DIR}/deploy.yml" --overrides \
