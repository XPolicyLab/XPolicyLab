...
from vlm_orchestrator.protocols.base import Backend, Frontend, FrontendSession
from vlm_orchestrator.strategies.base import (
    OrchestrationStrategy,
    SessionState,
    StrategyContext,
)
from vlm_orchestrator.strategies.subgoal_base import SubgoalBaseStrategy
...
from vlm_orchestrator.vlm.api import vla_coordinate_payload
from vlm_orchestrator.vlm.task_descriptions import find_task_reference
from vlm_orchestrator.robodojo_camera import fixed_head_calibration
...
class ProxyConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.frontend is None or self.backend is None:
            from vlm_orchestrator.protocols.robodojo_ws import RoboDojoWsFrontend
            from vlm_orchestrator.protocols.xpolicylab_ws import XPolicyLabWsBackend
        ...
        try:
            ...
            while True:
                ...
                async def process_item(item: dict) -> dict:
                    ...
                    processed, state = await asyncio.to_thread(
                ...
                candidate_groups = []
                for item in items:
                    count = item["sampled_candidate_count"]
                    candidate_groups.append(parse_candidates(raw_responses[response_offset:response_offset + count]))
                    response_offset += count

                async def select_item(item_index: int, item: dict, candidate_responses: list[dict]) -> None:
                    state = item["state"]
                    ...
                    if grouped and callable(apply_progress):
                        ...
                        updated_obs, state, changed = await asyncio.to_thread(
                ...
                await asyncio.gather(*(
                    select_item(index, item, candidates)
                    for index, (item, candidates) in enumerate(zip(items, candidate_groups, strict=True))
                ))
