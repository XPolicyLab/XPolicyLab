...
命令（`expert_data_num` 为可选尾参，留空 = 全部 episode）：
...
conda activate XPolicyLab
bash policy/starVLA/process_data.sh RoboDojo stack_bowls arx_x5 joint
bash policy/starVLA/process_data.sh RoboDojo stack_bowls arx_x5 joint 50
```
...
```text
policy/starVLA/checkpoints/RoboDojo-cotrain-arx_x5-joint-0
```

## 推理


命令（10 个参数，`ckpt_name` 直接是 `checkpoints/` 下完整的 run 目录名，历史 6 元组目录名可整体传入）：
...
不指定 ckpt：在policy/starvla下新建checkpoints文件夹，然后把final_ckpt中的RoboDojo-cotrain-arx_x5-joint-0文件夹拷贝到这个目录下
...
bash eval.sh RoboDojo stack RoboDojo-cotrain-arx_x5-joint-0 arx_x5 joint 0 0 1 XPolicyLab XPolicyLab
```

默认会按 `ckpt_name` 查找训练目录，并从 `checkpoints/` 下读取唯一的权重文件：

```text
policy/starVLA/checkpoints/RoboDojo-cotrain-arx_x5-joint-0/checkpoints/<checkpoint>.pt
```

也兼容旧的 `policy/starVLA/results/Checkpoints/<ckpt_name>/checkpoints/` 目录。
...
export STARVLA_CKPT_PATH=/cpfs_infra/user/wangkaixuan/chengy/demo_env/XPolicyLab/policy/starVLA/checkpoints/RoboDojo-stack_bowls-arx_x5-joint-0/final_model/pytorch_model.pt

bash eval.sh RoboDojo stack RoboDojo-stack_bowls-arx_x5-joint-0 arx_x5 joint 0 0 1 XPolicyLab XPolicyLab
```
